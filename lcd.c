...
//most of the bitfidding will be here
//primary task: sending a 4bit nibble to the LCD and create the Enable signal transition from 0 to 1 to 0
...
void lcd_init(void)
{
    ...
    DDRB |= CTRL_BITS;         // Set PB0 and PB1 to output
    DDRD |= DATA_BITS;         // Set PD4, PD5, PD6, PD7 to output 
...
void lcd_writenibble(unsigned char lcdbits)
{
    ...
    PORTD &= 0x0F; // clear upper bits to 0
    PORTD |= (lcdbits & 0xF0);  // set bits to 1


    /* Make E signal (PB1) go to 1 and back to 0 */
    PORTB |= (1 << PB1); // set PB1 to 1
    PORTB |= (1 << PB1); // set PB1 to 1
